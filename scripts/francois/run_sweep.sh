...
function RunMeasure() {
      function Run() {
          ...
          RemoteExec $DATA_PLANE "cd loader; sudo /usr/local/go/bin/go run cmd/loader.go  --config config_$1_sweep.json --verbosity trace"
...
# ./run_sweep.sh 2 3 4 5 10 25 50 100
          ...
          scp Francois@amd204.utah.cloudlab.us:~/cluster_manager/cmd/data_plane/data/proxy_trace.csv proxy_trace_100.csv
          scp Francois@amd195.utah.cloudlab.us:~/cluster_manager/cmd/master_node/data/cold_start_trace.csv cold_start_trace_100.csv